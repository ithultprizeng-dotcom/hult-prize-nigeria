import { ReactElement } from "react";

export function PrecisionFarmingSolution() {
  return (
    <svg
      fill="#ec2088"
      viewBox="0 0 96 96"
      data-name="Your Icons"
      id="Your_Icons"
      className="size-10"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <title></title>
        <path d="M74.61,22.66q-.36,0-.72,0a49,49,0,0,1-6-1l-1.35-.33.07-1.39c.09-2,1.77-7.06,8.29-7.06H75c7,.06,10.27,3.47,10.63,3.86l1.45,1.6-1.9,1C84.94,19.44,78.91,22.66,74.61,22.66Zm-6-2.94a45.84,45.84,0,0,0,5.44.92c3.19.31,8.34-2.17,9.79-2.9C83,17,80.15,14.86,75,14.82h-.1C69.73,14.82,68.8,18.65,68.64,19.72Z"></path>
        <path d="M73.59,36.34l-2.16-.12c-.81,0-8-.48-11.39-3a51.39,51.39,0,0,1-4.61-4l-1-1,.77-1.15c0-.06,4-5.89,10.82-1.68,5.91,3.66,7,8.31,7.13,8.83ZM57,28a46.31,46.31,0,0,0,4.2,3.63c2.26,1.69,7.2,2.42,9.86,2.61-.36-1.14-1.73-4.41-6.11-7.12S57.74,27.06,57,28Z"></path>
        <path d="M54,29c-2.15,0-6.11-.8-7.62-6.35-1.84-6.75.51-10.9.78-11.35l1.14-1.86L49.81,11c.22.23,5.48,5.76,6.27,10.05a49.09,49.09,0,0,1,.65,6.11l.06,1.38-1.35.32A7.59,7.59,0,0,1,54,29ZM48.68,12.76c-.52,1.09-1.72,4.41-.36,9.4,1.4,5.14,5.26,5,6.4,4.84a46.84,46.84,0,0,0-.61-5.55C53.53,18.28,49.79,14,48.68,12.76Z"></path>
        <path d="M59.13,92H36.92A9.93,9.93,0,0,1,27,82V41.73a1,1,0,0,1,2,0V82a7.93,7.93,0,0,0,7.88,8H59.13A7.93,7.93,0,0,0,67,82V41.73a1,1,0,0,1,2,0V82A9.93,9.93,0,0,1,59.13,92Z"></path>
        <path d="M48,47.21c-10.1,0-21-1.89-21-6s10.88-6,21-6,21,1.89,21,6S58.13,47.21,48,47.21Zm0-10.08c-12.28,0-19,2.67-19,4s6.7,4,19,4,19-2.67,19-4S60.31,37.13,48,37.13Z"></path>
        <path d="M48,43.85a.9.9,0,0,1-.28,0,1,1,0,0,1-.67-1.24c.28-1,7-23.11,20.94-23.11a1,1,0,0,1,0,2c-12.42,0-19,21.46-19,21.68A1,1,0,0,1,48,43.85Z"></path>
        <path d="M20.94,22.66c-4.3,0-10.32-3.22-10.59-3.36l-1.9-1,1.45-1.6c.36-.39,3.68-3.8,10.64-3.86h.11c6.53,0,8.2,5.09,8.3,7.06L29,21.27l-1.35.33a48.58,48.58,0,0,1-6,1Q21.32,22.66,20.94,22.66Zm-9.25-4.92c1.44.73,6.59,3.2,9.78,2.9a46.1,46.1,0,0,0,5.45-.92c-.16-1.06-1.09-4.9-6.27-4.9h-.1C15.41,14.86,12.57,17,11.69,17.74Z"></path>
        <path d="M22,36.34l.43-2.13c0-.21,1.08-5.09,7.12-8.83,6.79-4.21,10.79,1.62,10.83,1.68l.77,1.15-1,1a48.93,48.93,0,0,1-4.61,4c-3.42,2.55-10.59,3-11.39,3ZM34.33,25.86a7.09,7.09,0,0,0-3.76,1.22c-4.4,2.72-5.75,6-6.1,7.12,2.66-.19,7.59-.92,9.85-2.61A46.5,46.5,0,0,0,38.53,28,5.46,5.46,0,0,0,34.33,25.86Z"></path>
        <path d="M36,23.45a6,6,0,0,1-1.41-.15L33.21,23l.06-1.4,1,.05-1-.06a49.52,49.52,0,0,1,.65-6.09c.79-4.29,6-9.82,6.27-10L41.7,3.87l1.13,1.86c.28.45,2.62,4.6.78,11.35C42.19,22.31,38.38,23.45,36,23.45Zm-1.7-1.81h0Zm1-.24c1.06.13,5,.27,6.4-4.84s.16-8.3-.35-9.4c-1.12,1.23-4.86,5.53-5.44,8.69A45.59,45.59,0,0,0,35.28,21.4Z"></path>
        <path d="M47.53,43.85a1,1,0,0,1-1-.71c-.06-.22-6.61-21.68-19-21.68a1,1,0,0,1,0-2c13.91,0,20.67,22.16,21,23.11a1,1,0,0,1-.68,1.24A.9.9,0,0,1,47.53,43.85Z"></path>
        <path d="M50.94,64.19H45.11a2.12,2.12,0,0,1-2.14-2c-.14-2.54-.84-3.09-1.8-3.85-.25-.2-.51-.4-.77-.64a5.58,5.58,0,0,0-2.29-.67,10.86,10.86,0,0,1-5.9-2.42c-2.95-2.74-5.05-10.7-5.14-11A1,1,0,1,1,29,43.1c.56,2.15,2.48,8.14,4.57,10.08a9.12,9.12,0,0,0,4.88,1.92,6.33,6.33,0,0,1,3.31,1.17c.22.2.44.37.65.54,1.11.88,2.36,1.86,2.56,5.31a.16.16,0,0,0,.14.07h5.83a.16.16,0,0,0,.14-.07c.2-3.45,1.45-4.43,2.56-5.31.21-.17.43-.34.65-.54A6.33,6.33,0,0,1,57.6,55.1a9.12,9.12,0,0,0,4.88-1.92c2.1-2,4-7.93,4.56-10.08a1,1,0,0,1,1.22-.71A1,1,0,0,1,69,43.61c-.09.34-2.19,8.3-5.15,11a10.8,10.8,0,0,1-5.89,2.42,5.69,5.69,0,0,0-2.3.67,9,9,0,0,1-.77.64c-.95.76-1.65,1.31-1.79,3.85A2.12,2.12,0,0,1,50.94,64.19Z"></path>
        <path d="M58,82.5a8.11,8.11,0,0,1-5.74-2.32,5.94,5.94,0,0,0-8.52,0,8.24,8.24,0,0,1-11.46,0A5.91,5.91,0,0,0,28,78.38a1,1,0,1,1,0-2.06,8.09,8.09,0,0,1,5.73,2.32,5.94,5.94,0,0,0,8.52,0,8.24,8.24,0,0,1,11.46,0,6,6,0,0,0,8.53,0A8.09,8.09,0,0,1,68,76.32a1,1,0,1,1,0,2.06,5.91,5.91,0,0,0-4.26,1.8A8.09,8.09,0,0,1,58,82.5Z"></path>
        <path d="M41.08,76.24a.54.54,0,0,1-.35-.14.51.51,0,0,1,0-.71s1.45-1.49,1.06-2.79a4.31,4.31,0,0,1,1.31-5.08c1.56-1.44,1.17-4.29,1.16-4.32a.51.51,0,0,1,.43-.56.5.5,0,0,1,.56.42c0,.14.47,3.39-1.47,5.19a3.32,3.32,0,0,0-1,4.06c.57,1.88-1.23,3.71-1.3,3.78A.51.51,0,0,1,41.08,76.24Z"></path>
        <path d="M55.15,76.24a.51.51,0,0,1-.36-.15c-.07-.07-1.87-1.9-1.3-3.78a3.32,3.32,0,0,0-1-4.06c-1.94-1.8-1.49-5-1.47-5.19a.5.5,0,0,1,.56-.42.5.5,0,0,1,.43.56s-.39,2.88,1.16,4.32a4.31,4.31,0,0,1,1.32,5.08c-.4,1.3,1,2.77,1,2.79a.5.5,0,0,1,0,.71A.54.54,0,0,1,55.15,76.24Z"></path>
        <path d="M44.81,74.44l-.12,0a.48.48,0,0,1-.36-.6c.1-.42.19-.72.26-.94l.08-.26-.12-.09c-.12-.1-.28-.23-.47-.41-.94-.87-.25-1.9.2-2.58a2.7,2.7,0,0,0,.54-1.14c0-1.21,1.4-5,1.56-5.39a.5.5,0,1,1,.93.35,26.42,26.42,0,0,0-1.49,5,3.26,3.26,0,0,1-.71,1.69c-.55.83-.61,1.06-.35,1.29s.32.28.42.37c.58.48.58.71.37,1.43-.07.2-.15.48-.25.88A.5.5,0,0,1,44.81,74.44Z"></path>
        <path d="M47.8,72.31a.48.48,0,0,1-.36-.16.49.49,0,0,1,0-.7c.49-.45.42-.75.09-1.82a13.69,13.69,0,0,1-.43-1.58c-.31-1.51-.22-1.9.21-2.38a1.43,1.43,0,0,0,.25-.35,3.2,3.2,0,0,0-.21-1.95.5.5,0,0,1,.94-.36,4.08,4.08,0,0,1,.16,2.76,2.71,2.71,0,0,1-.39.57c-.15.16-.24.26,0,1.51a15.17,15.17,0,0,0,.4,1.48c.33,1,.61,2-.36,2.85A.47.47,0,0,1,47.8,72.31Z"></path>
        <path d="M50.94,75.06a.54.54,0,0,1-.25-.06.52.52,0,0,1-.19-.69,2,2,0,0,0,.19-.89,6.35,6.35,0,0,1,.48-2.14c.43-1,.43-1-.19-1.58-.12-.12-.27-.24-.43-.39-1.42-1.33-1.2-5.71-1.17-6.21a.48.48,0,0,1,.53-.47.5.5,0,0,1,.46.53c-.1,1.69.05,4.65.86,5.41l.42.38c1.09,1,1.07,1.22.44,2.71a5.46,5.46,0,0,0-.4,1.84,2.94,2.94,0,0,1-.32,1.31A.5.5,0,0,1,50.94,75.06Z"></path>
        <path d="M62.32,42.53c-1.14,0-2-.59-2-1.36s.86-1.35,2-1.35,2,.58,2,1.35S63.46,42.53,62.32,42.53Zm0-1.71c-.63,0-1,.26-1,.35s.37.36,1,.36,1-.27,1-.36S63,40.82,62.32,40.82Z"></path>
        <path d="M53,40c-1.14,0-2-.58-2-1.35s.86-1.36,2-1.36,2,.58,2,1.36S54.1,40,53,40Zm0-1.71c-.63,0-1,.26-1,.36s.37.35,1,.35,1-.26,1-.35S53.58,38.28,53,38.28Z"></path>
        <path d="M42.13,40.82c-1.45,0-2.54-.72-2.54-1.67s1.09-1.67,2.54-1.67,2.54.72,2.54,1.67S43.58,40.82,42.13,40.82Zm0-2.34c-.93,0-1.54.4-1.54.67s.61.67,1.54.67,1.54-.41,1.54-.67S43.06,38.48,42.13,38.48Z"></path>
        <path d="M32.62,41.9c-1,0-1.74-.59-1.74-1.33s.76-1.32,1.74-1.32,1.75.58,1.75,1.32S33.6,41.9,32.62,41.9Zm0-1.65c-.46,0-.74.22-.74.32s.28.33.74.33.75-.23.75-.33S33.09,40.25,32.62,40.25Z"></path>
        <path d="M36.5,41.23a1.24,1.24,0,0,1-.92-.36,1.16,1.16,0,0,1-.32-.87,1.8,1.8,0,0,1,.68-1.25,1.65,1.65,0,0,1,2.2-.13h0a1.16,1.16,0,0,1,.32.87,1.8,1.8,0,0,1-.68,1.25A2,2,0,0,1,36.5,41.23Zm.72-2a1,1,0,0,0-.64.26.81.81,0,0,0-.32.53c0,.09,0,.11,0,.12s.5.09.85-.2a.81.81,0,0,0,.32-.53c0-.08,0-.11,0-.12h0A.31.31,0,0,0,37.22,39.26Z"></path>
        <path d="M44.52,44.36a2,2,0,0,1-1.32-.56,1.78,1.78,0,0,1-.61-1.28,1.11,1.11,0,0,1,.36-.85h0a1.64,1.64,0,0,1,2.19.23,1.78,1.78,0,0,1,.61,1.28,1.13,1.13,0,0,1-.36.86A1.28,1.28,0,0,1,44.52,44.36Zm-.89-2a.15.15,0,0,0,0,.12.8.8,0,0,0,.29.55c.32.3.71.34.83.23a.15.15,0,0,0,0-.12.8.8,0,0,0-.29-.55C44.13,42.33,43.74,42.29,43.63,42.4Z"></path>
        <path d="M52.24,42.93a2,2,0,0,1-1.32-.56,1.76,1.76,0,0,1-.61-1.27,1.17,1.17,0,0,1,.36-.86h0a1.65,1.65,0,0,1,2.19.23,1.8,1.8,0,0,1,.61,1.28,1.13,1.13,0,0,1-.36.86A1.28,1.28,0,0,1,52.24,42.93Zm-.89-2a.16.16,0,0,0,0,.13.82.82,0,0,0,.29.54c.33.3.72.34.83.24a.16.16,0,0,0,0-.13.78.78,0,0,0-.29-.54C51.85,40.9,51.46,40.87,51.35,41Z"></path>
        <path d="M57.58,44.13a2,2,0,0,1-1.32-.55,1.82,1.82,0,0,1-.62-1.28,1.17,1.17,0,0,1,.36-.86,1.66,1.66,0,0,1,2.2.24A1.76,1.76,0,0,1,58.81,43a1.17,1.17,0,0,1-.36.86A1.23,1.23,0,0,1,57.58,44.13Zm-.71-2a.33.33,0,0,0-.19.05h0a.16.16,0,0,0,0,.13.8.8,0,0,0,.3.54c.32.3.71.34.83.24a.16.16,0,0,0,0-.13.82.82,0,0,0-.29-.54A1,1,0,0,0,56.87,42.12Zm-.53-.31h0Z"></path>
        <path d="M39.35,43.35A1.33,1.33,0,1,1,40.59,42,1.28,1.28,0,0,1,39.35,43.35Zm0-1.65c-.12,0-.23.15-.23.33s.11.32.23.32.24-.15.24-.32S39.48,41.7,39.35,41.7Z"></path>
        <path d="M58.65,40.81a1.77,1.77,0,0,1-.62-.13,1.44,1.44,0,0,1-.8-.79,1.09,1.09,0,0,1,0-.85A1.32,1.32,0,0,1,59,38.52h0a1.38,1.38,0,0,1,.8.79,1,1,0,0,1,0,.85A1.24,1.24,0,0,1,58.65,40.81Zm-.24-1.42c-.13,0-.22,0-.24.09a.39.39,0,0,0,.25.28.44.44,0,0,0,.47,0,.39.39,0,0,0-.25-.28A.56.56,0,0,0,58.41,39.39Z"></path>
        <path d="M76.34,92H28.5a1,1,0,0,1,0-2H76.34a1,1,0,0,1,0,2Z"></path>
        <path d="M25.72,92H21.65a1,1,0,0,1,0-2h4.07a1,1,0,0,1,0,2Z"></path>
        <path d="M18.32,92H16a1,1,0,0,1,0-2h2.31a1,1,0,0,1,0,2Z"></path>
      </g>
    </svg>
  );
}

export function ColdChainProduce() {
  return (
    <svg
      fill="#ec2088"
      viewBox="0 0 100 100"
      version="1.1"
      className="size-10"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g id="store"></g> <g id="groceries"></g> <g id="fruits"></g>{" "}
        <g id="bakery"></g> <g id="hygiene_product"></g> <g id="detergent"></g>{" "}
        <g id="cooking_oil"></g> <g id="snacks"></g> <g id="canned_food"></g>{" "}
        <g id="wheat_flour"></g> <g id="milk_powder"></g>{" "}
        <g id="coffee_powder"></g> <g id="tea_bag"></g> <g id="meat"></g>{" "}
        <g id="chicken"></g> <g id="cart"></g> <g id="sausage"></g>{" "}
        <g id="egg"></g> <g id="drink"></g>{" "}
        <g id="vegetable">
          {" "}
          <g>
            {" "}
            <path d="M88,48h-4.5c1.6-6,2.9-11.4,3.2-13.3c0.9-5.4-1.8-10.6-6.2-13.2c1.1-0.3,2.2-0.8,3.3-1.5c3.4-2.5,4.2-7,4.2-7.2 c0-0.3,0-0.5-0.2-0.8c-0.2-0.2-0.4-0.4-0.7-0.4c-0.2,0-3.3-0.4-6.2,0.9c-0.2-3.3-2-5.9-2.1-6c-0.2-0.2-0.4-0.4-0.7-0.4 c-0.3,0-0.5,0-0.8,0.2c-0.1,0.1-2.6,2-3.8,5.1c-2.4-2.1-5.5-2.7-5.7-2.8c-0.3,0-0.5,0-0.8,0.2s-0.4,0.4-0.4,0.7 c0,0.2-0.6,4.7,1.8,8.1c0.8,1.1,1.7,1.9,2.6,2.5c-1.7,0.4-3.3,1.1-4.7,2.1c-1.8,1.3-3.2,3-4.1,5c-1.2-3.7-4-6.7-7.6-8.1 c1-0.5,2.1-1.1,3-2c3-3,3.1-7.6,3.1-7.8c0-0.3-0.1-0.5-0.3-0.7s-0.5-0.3-0.7-0.3c-0.2,0-3.3,0.1-6.1,1.8c-0.7-3.2-2.9-5.5-3-5.6 c-0.4-0.4-1.1-0.4-1.4,0c-0.1,0.1-2.3,2.4-3,5.6c-2.8-1.7-6-1.8-6.1-1.8c-0.3,0-0.5,0.1-0.7,0.3c-0.2,0.2-0.3,0.5-0.3,0.7 c0,0.2,0.1,4.8,3.1,7.8c0.9,0.9,2,1.6,3,2c-3.6,1.4-6.4,4.4-7.6,8.1c-0.9-2-2.3-3.7-4.1-5c-1.4-1-3-1.8-4.7-2.1 c0.9-0.6,1.9-1.4,2.6-2.5c2.4-3.4,1.9-7.9,1.8-8.1c0-0.3-0.2-0.5-0.4-0.7c-0.2-0.2-0.5-0.2-0.8-0.2c-0.2,0-3.2,0.6-5.7,2.8 c-1.2-3-3.7-5-3.8-5.1C22.4,6,22.1,6,21.8,6c-0.3,0-0.5,0.2-0.7,0.4c-0.1,0.1-1.9,2.8-2.1,6c-3-1.3-6.1-0.9-6.2-0.9 c-0.3,0-0.5,0.2-0.7,0.4C12,12.2,12,12.5,12,12.8c0,0.2,0.9,4.7,4.2,7.2c1,0.8,2.2,1.2,3.3,1.5c-4.4,2.6-7,7.8-6.2,13.2 c0.3,1.9,1.6,7.3,3.2,13.3H12c-0.6,0-1,0.4-1,1v6c0,0.6,0.4,1,1,1h1.1L17,86.3c0.7,5.5,5.4,9.7,11,9.7h44.2c5.5,0,10.3-4.2,11-9.7 L86.9,56H88c0.6,0,1-0.4,1-1v-6C89,48.4,88.6,48,88,48z M82.6,18.3c-1.4,1-3,1.4-4.3,1.5c1-1.2,2.1-2.9,2.5-5 c1.7-1,3.7-1.2,4.9-1.2C85.3,14.8,84.4,17,82.6,18.3z M77.7,8.6c0.6,1.2,1.4,3.4,1.1,5.6c-0.3,2.2-1.8,4-2.8,5 c-0.6-1.2-1.4-3.4-1.1-5.6C75.3,11.4,76.8,9.6,77.7,8.6z M70.2,16.3c-1.3-1.8-1.5-4.2-1.5-5.5c1.2,0.4,3,1.2,4.3,2.7 c-0.3,2.1,0.3,4.1,0.9,5.6C72.6,18.6,71.2,17.7,70.2,16.3z M67.7,23.7c2.3-1.7,5.2-2.4,8-1.9c5.9,1,9.9,6.6,9,12.6 c-0.3,1.6-1.5,6.8-3.3,13.6h-1.9c0.1-0.1,0.1-0.2,0.1-0.3c0.1-0.5-0.3-1.1-0.8-1.1l-5.8-0.9c-0.5-0.1-1.1,0.3-1.1,0.8 s0.3,1.1,0.8,1.1l2.7,0.4H62c0.4-8.1,0.9-15,1.2-17.1C63.7,28,65.3,25.4,67.7,23.7z M56.4,15.5c-1.2,1.2-2.8,1.9-4.1,2.2 c0.8-1.3,1.7-3.2,1.7-5.4c1.6-1.3,3.5-1.8,4.8-2C58.6,11.6,58,13.9,56.4,15.5z M50,6.6c0.8,1.1,2,3.1,2,5.4c0,2.3-1.2,4.3-2,5.4 c-0.8-1.1-2-3.1-2-5.4C48,9.7,49.2,7.7,50,6.6z M41.2,10.2c1.2,0.2,3.2,0.7,4.8,2c0.1,2.2,0.9,4.1,1.7,5.4c-1.3-0.3-2.9-1-4.1-2.2 C42,13.9,41.4,11.6,41.2,10.2z M50,20C50,20,50,20,50,20C50,20,50,20,50,20c6.1,0,11,4.9,11,11c0,2.1-0.5,8.9-1.4,17H40.4 C39.5,39.9,39,33.1,39,31C39,24.9,43.9,20,50,20z M31.3,10.8c0,1.4-0.2,3.7-1.5,5.5c-1,1.4-2.4,2.2-3.6,2.8 c0.6-1.5,1.1-3.5,0.9-5.6C28.3,12,30.2,11.2,31.3,10.8z M22.3,8.6c1,1,2.4,2.8,2.8,5c0.3,2.2-0.5,4.4-1.1,5.6c-1-1-2.4-2.8-2.8-5 C20.8,12,21.6,9.8,22.3,8.6z M14.3,13.5c1.2,0,3.2,0.2,4.9,1.2c0.4,2.1,1.5,3.9,2.5,5.1c-1.3-0.1-3-0.5-4.3-1.5 C15.6,17,14.7,14.8,14.3,13.5z M15.3,34.4c-0.9-6,3.1-11.6,9-12.6c2.8-0.5,5.7,0.2,8,1.9c2.4,1.7,3.9,4.3,4.4,7.2 c0.3,2.1,0.9,9,1.2,17.1H24.7l2.7-0.4c0.5-0.1,0.9-0.6,0.8-1.1s-0.6-0.9-1.1-0.8l-5.8,0.9c-0.5,0.1-0.9,0.6-0.8,1.1 c0,0.1,0.1,0.2,0.1,0.3h-1.9C16.8,41.2,15.6,36,15.3,34.4z M81.1,86.1c-0.6,4.5-4.4,7.9-9,7.9H27.9c-4.5,0-8.4-3.4-9-7.9L15.1,56 H40h20h24.9L81.1,86.1z M87,54h-1H60H40H14h-1v-4h74V54z"></path>{" "}
            <path d="M43,32h6c0.6,0,1-0.4,1-1s-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1S42.4,32,43,32z"></path>{" "}
            <path d="M52,40h6c0.6,0,1-0.4,1-1s-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1S51.4,40,52,40z"></path>{" "}
            <path d="M49,44h-6c-0.6,0-1,0.4-1,1s0.4,1,1,1h6c0.6,0,1-0.4,1-1S49.6,44,49,44z"></path>{" "}
            <path d="M26,32.6c-0.1-0.5-0.6-0.9-1.1-0.8l-5.8,0.9c-0.5,0.1-0.9,0.6-0.8,1.1c0.1,0.5,0.5,0.8,1,0.8c0.1,0,0.1,0,0.2,0l5.8-0.9 C25.8,33.7,26.1,33.2,26,32.6z"></path>{" "}
            <path d="M29.2,41.2c0.1,0,0.1,0,0.2,0l5.8-0.9c0.5-0.1,0.9-0.6,0.8-1.1s-0.6-0.9-1.1-0.8l-5.8,0.9c-0.5,0.1-0.9,0.6-0.8,1.1 C28.3,40.8,28.7,41.2,29.2,41.2z"></path>{" "}
            <path d="M74.8,33.8l5.8,0.9c0.1,0,0.1,0,0.2,0c0.5,0,0.9-0.3,1-0.8c0.1-0.5-0.3-1.1-0.8-1.1l-5.8-0.9c-0.5-0.1-1.1,0.3-1.1,0.8 C73.9,33.2,74.2,33.7,74.8,33.8z"></path>{" "}
            <path d="M64.8,40.2l5.8,0.9c0.1,0,0.1,0,0.2,0c0.5,0,0.9-0.3,1-0.8c0.1-0.5-0.3-1.1-0.8-1.1l-5.8-0.9c-0.5-0.1-1.1,0.3-1.1,0.8 C63.9,39.6,64.2,40.1,64.8,40.2z"></path>{" "}
          </g>{" "}
        </g>{" "}
      </g>
    </svg>
  );
}

export function VocationalEdtech() {
  return (
    <svg
      viewBox="0 0 1024 1024"
      className="icon size-10"
      version="1.1"
      xmlns="http://www.w3.org/2000/svg"
      fill="#ec2088"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <path
          d="M981.333333 469.333333a21.333333 21.333333 0 0 0-21.333333 21.333334v85.333333a85.333333 85.333333 0 0 1-85.333333 85.333333H149.333333a85.333333 85.333333 0 0 1-85.333333-85.333333V149.333333a85.333333 85.333333 0 0 1 85.333333-85.333333h725.333334a85.333333 85.333333 0 0 1 85.333333 85.333333 21.333333 21.333333 0 0 0 42.666667 0 128 128 0 0 0-128-128H149.333333a128 128 0 0 0-128 128v426.666667a128 128 0 0 0 128 128h725.333334a128 128 0 0 0 128-128v-85.333333a21.333333 21.333333 0 0 0-21.333334-21.333334z"
          fill="#ec2088"
        ></path>
        <path
          d="M981.333333 298.666667a21.333333 21.333333 0 0 0-21.333333 21.333333v85.333333a21.333333 21.333333 0 0 0 42.666667 0v-85.333333a21.333333 21.333333 0 0 0-21.333334-21.333333zM966.186667 219.52a21.333333 21.333333 0 0 0 0 30.293333 21.333333 21.333333 0 0 0 30.293333 0l2.56-3.413333a11.946667 11.946667 0 0 0 1.92-3.626667 13.226667 13.226667 0 0 0 1.706667-3.84 26.88 26.88 0 0 0 0-4.266666 21.333333 21.333333 0 0 0-36.48-15.146667zM896 960H128a21.333333 21.333333 0 0 0 0 42.666667h768a21.333333 21.333333 0 0 0 0-42.666667zM682.666667 853.333333a21.333333 21.333333 0 0 0 0-42.666666H341.333333a21.333333 21.333333 0 0 0 0 42.666666zM256 576a21.333333 21.333333 0 0 0 21.333333 21.333333h258.773334a21.333333 21.333333 0 0 0 13.226666-4.693333l134.4-106.666667 1.92-1.92 1.493334-1.493333a21.333333 21.333333 0 0 0 2.773333-4.693333l109.866667-277.333334A21.333333 21.333333 0 0 0 789.333333 170.666667l-158.506666-64a21.333333 21.333333 0 0 0-27.733334 11.946666l-109.866666 277.333334A21.333333 21.333333 0 0 0 490.666667 405.333333v3.413334L512 554.666667H277.333333a21.333333 21.333333 0 0 0-21.333333 21.333333z m282.24-136.96l90.24 35.84-76.586667 61.013333zM633.813333 155.733333l119.04 47.146667-94.293333 238.08-118.826667-47.146667z"
          fill="#ec2088"
        ></path>
      </g>
    </svg>
  );
}

export function LanguageLearning() {
  return (
    <svg
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      fill="#ec2088"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <path d="M8 15H3.5A2.502 2.502 0 0 1 1 12.5v-9A2.502 2.502 0 0 1 3.5 1h9A2.502 2.502 0 0 1 15 3.5V8h-1V3.5A1.502 1.502 0 0 0 12.5 2h-9A1.502 1.502 0 0 0 2 3.5v9A1.502 1.502 0 0 0 3.5 14H8zm-.038-4.811a9.77 9.77 0 0 1-3.766 1.796l-.242-.97a8.816 8.816 0 0 0 3.282-1.532A9.264 9.264 0 0 1 4.888 5H4V4h3.279l-.544-.544.707-.707L8.692 4H12v1h-.914A9.836 9.836 0 0 1 9.78 8.152a3.853 3.853 0 0 0-1.82 2.037zm.032-1.383A8.167 8.167 0 0 0 10.058 5H5.922a8.18 8.18 0 0 0 2.072 3.806zM23 20.447v-8.894A2.525 2.525 0 0 0 20.484 9h-8.931A2.556 2.556 0 0 0 9 11.553v8.894A2.556 2.556 0 0 0 11.553 23h8.894A2.556 2.556 0 0 0 23 20.447zM20.484 10A1.517 1.517 0 0 1 22 11.516v8.968A1.517 1.517 0 0 1 20.484 22h-8.968A1.517 1.517 0 0 1 10 20.484v-8.968A1.517 1.517 0 0 1 11.516 10zm-2.086 8h-4.796l-1.159 2.23-.886-.46L16 11.215l4.443 8.555-.886.46zm-.52-1L16 13.385 14.122 17zM6 22.01a2.003 2.003 0 0 1-2-2v-2.303l1.646 1.646.707-.707L3.506 15.8.659 18.646l.707.707L3 17.72v2.292a3.003 3.003 0 0 0 3 3h2.058v-1zM22.646 4.647L21 6.293V4a3.003 3.003 0 0 0-3-3h-2v1h2a2.003 2.003 0 0 1 2 2v2.281l-1.634-1.635-.707.707 2.847 2.848 2.848-2.848z"></path>
        <path fill="none" d="M0 0h24v24H0z"></path>
      </g>
    </svg>
  );
}

export function OffGridPowerSolution() {
  return (
    <svg
      version="1.1"
      id="Layer_1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="#000000"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g>
          {" "}
          <rect
            x="109.409"
            y="430.926"
            style={{ fill: "#ffffff" }}
            width="293.166"
            height="66.703"
          ></rect>{" "}
          <rect
            x="178.483"
            y="14.369"
            style={{ fill: "#ffffff" }}
            width="155.032"
            height="53.776"
          ></rect>{" "}
          <circle
            style={{ fill: "#ffffff" }}
            cx="109.222"
            cy="196.273"
            r="25.632"
          ></circle>{" "}
          <circle
            style={{ fill: "#ffffff" }}
            cx="402.776"
            cy="196.273"
            r="25.632"
          ></circle>{" "}
        </g>{" "}
        <path
          style={{ fill: "#ec2088" }}
          d="M362.776,196.268c0,22.057,17.945,40.002,40.002,40.002c22.057,0,40.002-17.945,40.002-40.002 c0-16.992-10.656-31.535-25.632-37.322v-33.625c0-7.935-6.435-14.369-14.369-14.369h-68.269l-4.046-28.436h3.052 c7.935,0,14.369-6.435,14.369-14.369V14.369C347.883,6.435,341.449,0,333.514,0H178.486c-7.935,0-14.369,6.435-14.369,14.369v53.778 c0,7.935,6.435,14.369,14.369,14.369h3.055l-4.042,28.436h-68.277c-7.935,0-14.369,6.435-14.369,14.369v33.625 c-14.976,5.787-25.632,20.328-25.632,37.322c0,22.057,17.945,40.002,40.002,40.002s40.002-17.945,40.002-40.002 c0-16.992-10.656-31.535-25.632-37.322v-19.255h49.825l-39.352,276.87h-24.646c-7.935,0-14.369,6.435-14.369,14.369v66.7 c0,7.935,6.435,14.369,14.369,14.369h293.166c7.935,0,14.369-6.435,14.369-14.369v-66.7c0-7.935-6.435-14.369-14.369-14.369h-31.403 l-94.267-89.135l26.687-25.234c5.768-5.452,6.021-14.546,0.569-20.313c-5.453-5.771-14.548-6.024-20.313-0.569l-27.857,26.341 l-47.507-44.92h74.97c7.935,0,14.369-6.435,14.369-14.369c0-7.935-6.435-14.369-14.369-14.369h-94.422l2.999-21.102l63.96-60.478 l63.992,60.508l18.323,128.752c1.116,7.857,8.393,13.308,16.252,12.201c7.857-1.118,13.32-8.395,12.201-16.252l-28.17-197.929h49.81 v19.255C373.431,164.733,362.776,179.275,362.776,196.268z M109.222,207.531c-6.21,0-11.263-5.052-11.263-11.263 c0-6.21,5.052-11.263,11.263-11.263s11.263,5.052,11.263,11.263C120.485,202.479,115.433,207.531,109.222,207.531z M288.089,82.517 l-32.09,30.343l-32.09-30.343C223.909,82.517,288.089,82.517,288.089,82.517z M192.856,28.739h126.289v25.039H192.856V28.739z M388.213,483.261H123.787V445.3h22.748c0.006,0,0.013,0.001,0.02,0.001c0.01,0,0.02-0.001,0.03-0.001h241.628v37.961H388.213z M329.353,416.561H182.646l73.353-69.361L329.353,416.561z M166.551,392.23l16.239-114.256l52.296,49.45L166.551,392.23z M198.534,167.197l8.66-60.935l27.891,26.374L198.534,167.197z M276.913,132.635l27.898-26.379l8.676,60.962L276.913,132.635z M402.778,207.531c-6.21,0-11.263-5.052-11.263-11.263c0-6.21,5.052-11.263,11.263-11.263c6.211,0,11.263,5.052,11.263,11.263 C414.04,202.479,408.988,207.531,402.778,207.531z"
        ></path>{" "}
      </g>
    </svg>
  );
}

export function EnergyModel() {
  return (
    <svg
      fill="#ec2088"
      viewBox="0 0 16 16"
      id="energy-distribution-16px"
      xmlns="http://www.w3.org/2000/svg"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <path
          id="Path_118"
          data-name="Path 118"
          d="M-9.342,11.026l-3-1a.5.5,0,0,0-.316,0l-3,1A.5.5,0,0,0-16,11.5v4a.5.5,0,0,0,.5.5h6a.5.5,0,0,0,.5-.5v-4A.5.5,0,0,0-9.342,11.026ZM-13,14h1v1h-1Zm3,1h-1V13.5a.5.5,0,0,0-.5-.5h-2a.5.5,0,0,0-.5.5V15h-1V11.86l2.5-.833,2.5.833Zm-8.342-3.974-3-1a.5.5,0,0,0-.316,0l-3,1A.5.5,0,0,0-25,11.5v4a.5.5,0,0,0,.5.5h6a.5.5,0,0,0,.5-.5v-4A.5.5,0,0,0-18.342,11.026ZM-22,14h1v1h-1Zm3,1h-1V13.5a.5.5,0,0,0-.5-.5h-2a.5.5,0,0,0-.5.5V15h-1V11.86l2.5-.833,2.5.833Zm6.5-10a.5.5,0,0,1,.5.5V7.293l-1.146-1.147a.5.5,0,0,0-.708,0,.5.5,0,0,0,0,.708l2,2a.518.518,0,0,0,.163.109A.5.5,0,0,0-11.5,9a.5.5,0,0,0,.191-.038.518.518,0,0,0,.163-.109l2-2a.5.5,0,0,0,0-.708.5.5,0,0,0-.708,0L-11,7.293V5.5A1.5,1.5,0,0,0-12.5,4H-13a1,1,0,0,0-1-1h-.618l.562-1.684a1,1,0,0,0-.138-.9A1,1,0,0,0-15,0h-2a1,1,0,0,0-.97.757l-.713,2.853-1.463-1.464a.5.5,0,0,0-.708,0,.5.5,0,0,0,0,.708L-19.707,4H-22.5A1.5,1.5,0,0,0-24,5.5v3a.5.5,0,0,0,.5.5.5.5,0,0,0,.5-.5v-3a.5.5,0,0,1,.5-.5h2.793l-1.147,1.146a.5.5,0,0,0,0,.708A.5.5,0,0,0-20.5,7a.5.5,0,0,0,.354-.146l1.32-1.321a.765.765,0,0,0,.038.082A1,1,0,0,0-18,6h.719l-.689,2.757a1,1,0,0,0,.587,1.167A1.014,1.014,0,0,0-17,10a1,1,0,0,0,.858-.486L-13.434,5ZM-16,5h-2l1-4h2l-1,3h2l-3,5Z"
          transform="translate(25)"
        ></path>{" "}
      </g>
    </svg>
  );
}

export function WasteToWasteEcosystem() {
  return (
    <svg
      viewBox="0 0 128 128"
      xmlns="http://www.w3.org/2000/svg"
      fill="#ec2088"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <defs> </defs>{" "}
        <g id="Recycle">
          {" "}
          <path
            className="cls-1"
            d="M63.18,74.76a1,1,0,0,0,1.64,0L66.6,72.2a14.51,14.51,0,0,0,0-16l-1.78-2.57a1,1,0,0,0-1.64,0L61.4,56.19a14.51,14.51,0,0,0,0,16Zm-.13-17.43h0L64,56l1,1.38a12.57,12.57,0,0,1,0,13.73l-1,1.38-1-1.38A12.57,12.57,0,0,1,63.05,57.33Z"
          ></path>{" "}
          <path
            className="cls-1"
            d="M65.94,83.08a1,1,0,0,0,.28.89,1,1,0,0,0,.71.29h.18l3.07-.56A14.51,14.51,0,0,0,81.5,72.37l.56-3.07a1,1,0,0,0-.28-.88,1,1,0,0,0-.89-.28l-3.07.55A14.51,14.51,0,0,0,66.5,80Zm2.53-2.7a12.62,12.62,0,0,1,9.71-9.72h0l1.65-.3L79.53,72a12.61,12.61,0,0,1-9.71,9.71l-1.65.3Z"
          ></path>{" "}
          <path
            className="cls-1"
            d="M46.5,72.37A14.51,14.51,0,0,0,57.83,83.69l3.06.56h.18a1,1,0,0,0,.71-.29,1,1,0,0,0,.28-.89L61.5,80A14.51,14.51,0,0,0,50.18,68.69l-3.07-.55a1,1,0,0,0-.89.28,1,1,0,0,0-.28.88Zm3.32-1.71a12.62,12.62,0,0,1,9.71,9.72l.3,1.64-1.65-.3A12.61,12.61,0,0,1,48.47,72l-.3-1.65Z"
          ></path>{" "}
          <path
            className="cls-1"
            d="M95,20H73V16a1,1,0,0,0-1-1H56a1,1,0,0,0-1,1v4H34a4,4,0,0,0-4,4v1a4,4,0,0,0,3.09,3.89l9.46,83.22a1,1,0,0,0,1,.89h40.9a1,1,0,0,0,1-.89L94.89,29H95a4,4,0,0,0,4-4V24A4,4,0,0,0,95,20ZM57,17H71v3H57Zm26.56,94H44.44L35.12,29H92.88ZM97,25a2,2,0,0,1-2,2H34a2,2,0,0,1-2-2V24a2,2,0,0,1,2-2H95a2,2,0,0,1,2,2Z"
          ></path>{" "}
        </g>{" "}
      </g>
    </svg>
  );
}

export function WaterQualityAndMonitoring() {
  return (
    <svg
      fill="#ec2088"
      viewBox="0 0 1024 1024"
      xmlns="http://www.w3.org/2000/svg"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <path d="M766.545 909.463c-62.106 0-112.64-50.534-112.64-112.64 0-22.927 6.851-45.005 19.825-63.836l33.731 23.214c-8.233 11.981-12.595 26.02-12.595 40.622 0 39.516 32.164 71.68 71.68 71.68s71.68-32.164 71.68-71.68c0-15.36-4.792-30.013-13.844-42.342l33.024-24.238c14.254 19.425 21.78 42.445 21.78 66.58 0 62.106-50.534 112.64-112.64 112.64z"></path>
        <path d="M851.125 780.133c-7.096 0-13.998-3.686-17.787-10.291l-66.806-116.49-66.785 116.49c-5.632 9.81-18.145 13.21-27.965 7.578-9.81-5.622-13.199-18.145-7.578-27.955l69.284-120.812c7.772-13.517 19.804-21.279 33.044-21.279 13.251 0 25.293 7.762 33.055 21.268l69.284 120.822c5.622 9.81 2.232 22.333-7.578 27.955a20.36 20.36 0 01-10.168 2.714zM542.285 81.92c5.657 0 10.24-4.583 10.24-10.24V51.2c0-5.657-4.583-10.24-10.24-10.24h-256a10.238 10.238 0 00-10.24 10.24v20.48c0 5.657 4.583 10.24 10.24 10.24h256zm0 40.96h-256c-28.278 0-51.2-22.922-51.2-51.2V51.2c0-28.278 22.922-51.2 51.2-51.2h256c28.278 0 51.2 22.922 51.2 51.2v20.48c0 28.278-22.922 51.2-51.2 51.2z"></path>
        <path d="M347.73 263.335V81.913h133.12v181.422H347.73zm92.16-40.96v-99.502h-51.2v99.502h51.2z"></path>
        <path d="M277.504 981.333c5.657 0 10.24-4.583 10.24-10.24V449.437c0-45.246 36.674-81.92 81.92-81.92H637.44c45.246 0 81.92 36.674 81.92 81.92v66.908c0 11.309 9.171 20.48 20.48 20.48h62.464c11.309 0 20.48-9.171 20.48-20.48v-68.956c0-101.8-82.52-184.32-184.32-184.32H368.64c-101.8 0-184.32 82.52-184.32 184.32v523.704c0 5.657 4.583 10.24 10.24 10.24h82.944zm0 40.96H194.56c-28.278 0-51.2-22.922-51.2-51.2V447.389c0-124.422 100.858-225.28 225.28-225.28h269.824c124.422 0 225.28 100.858 225.28 225.28v68.956c0 33.931-27.509 61.44-61.44 61.44H739.84c-33.931 0-61.44-27.509-61.44-61.44v-66.908c0-22.625-18.335-40.96-40.96-40.96H369.664c-22.625 0-40.96 18.335-40.96 40.96v521.656c0 28.278-22.922 51.2-51.2 51.2z"></path>
      </g>
    </svg>
  );
}

export function MicroLendingForBusinesses() {
  return (
    <svg
      fill="#ec2088"
      viewBox="0 0 96 96"
      xmlns="http://www.w3.org/2000/svg"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <title></title>{" "}
        <g data-name="Payment History" id="Payment_History">
          {" "}
          <path d="M34,48.5A6.5,6.5,0,1,0,40.5,42,6.51,6.51,0,0,0,34,48.5Zm12,0A5.5,5.5,0,1,1,40.5,43,5.51,5.51,0,0,1,46,48.5Z"></path>{" "}
          <path d="M55,48.5A2.5,2.5,0,1,0,52.5,51,2.5,2.5,0,0,0,55,48.5ZM52.5,50A1.5,1.5,0,1,1,54,48.5,1.5,1.5,0,0,1,52.5,50Z"></path>{" "}
          <path d="M25,48.5A2.5,2.5,0,1,0,27.5,46,2.5,2.5,0,0,0,25,48.5Zm4,0A1.5,1.5,0,1,1,27.5,47,1.5,1.5,0,0,1,29,48.5Z"></path>{" "}
          <path d="M16.5,31a.5.5,0,0,0,.5-.5V27H64v3.5a.5.5,0,0,0,1,0v-4a.5.5,0,0,0-.5-.5h-48a.5.5,0,0,0-.5.5v4A.5.5,0,0,0,16.5,31Z"></path>{" "}
          <path d="M20.5,25a.5.5,0,0,0,.5-.5V21H60v3.5a.5.5,0,0,0,1,0v-4a.5.5,0,0,0-.5-.5h-40a.5.5,0,0,0-.5.5v4A.5.5,0,0,0,20.5,25Z"></path>{" "}
          <path d="M69.5,46A14.54,14.54,0,0,0,55.26,57.9L54,55.28a.5.5,0,0,0-.9.44l2,4,0,0,0,0a.39.39,0,0,0,.1.08l.06,0a.55.55,0,0,0,.24.07h0a.52.52,0,0,0,.43-.21L59.8,56.9a.5.5,0,0,0-.6-.8l-3,2.26a13.49,13.49,0,1,1,.54,6.48.51.51,0,0,0-.64-.31.5.5,0,0,0-.31.63A14.5,14.5,0,1,0,69.5,46Z"></path>{" "}
          <path d="M68.5,52a.5.5,0,0,0-.5.5v10a0,0,0,0,0,0,0,.44.44,0,0,0,0,.1.29.29,0,0,0,0,.09l0,.07a.27.27,0,0,0,.07.08l0,0,5,4a.51.51,0,0,0,.31.11.48.48,0,0,0,.39-.19.5.5,0,0,0-.08-.7L69,62.26V52.5A.5.5,0,0,0,68.5,52Z"></path>{" "}
          <path d="M49.5,63H13V34H68v9.55a.5.5,0,0,0,1,0v-10a.5.5,0,0,0-.5-.5h-56a.5.5,0,0,0-.5.5v30a.5.5,0,0,0,.5.5h37a.5.5,0,0,0,0-1Z"></path>{" "}
          <path d="M50.16,58H21.09A3.63,3.63,0,0,0,18,54.91V42.09A3.63,3.63,0,0,0,21.09,39H58.91A3.63,3.63,0,0,0,62,42.09v2.69a.5.5,0,0,0,1,0V41.63a.5.5,0,0,0-.5-.5,2.63,2.63,0,0,1-2.63-2.63.5.5,0,0,0-.5-.5H20.63a.5.5,0,0,0-.5.5,2.63,2.63,0,0,1-2.63,2.63.5.5,0,0,0-.5.5V55.37a.5.5,0,0,0,.5.5,2.63,2.63,0,0,1,2.63,2.63.5.5,0,0,0,.5.5H50.16a.5.5,0,0,0,0-1Z"></path>{" "}
        </g>{" "}
      </g>
    </svg>
  );
}

export function TelemedicineForRuralAreas() {
  return (
    <svg
      fill="#ec2088"
      viewBox="0 0 32 32"
      xmlns="http://www.w3.org/2000/svg"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <title></title>{" "}
        <g data-name="Layer 22" id="Layer_22">
          {" "}
          <path d="M29.47,15.27a7.91,7.91,0,0,0,.25-2.06,8,8,0,0,0-8-8h-.09A8,8,0,0,0,16,7.58a8.2,8.2,0,0,0-5.71-2.26,8.16,8.16,0,0,0-7.85,10,1,1,0,0,0-.43.83,1,1,0,0,0,1,1h.05a8.31,8.31,0,0,0,1.58,2.18l.43.39a1.07,1.07,0,0,0,.14.16l10.22,10a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.3l10-10.23a1.06,1.06,0,0,0,.24-.39l.32-.31a7.94,7.94,0,0,0,1.32-1.8H29a1,1,0,0,0,1-1A1,1,0,0,0,29.47,15.27ZM6,9.18a6.15,6.15,0,0,1,8.7-.09,5.38,5.38,0,0,1,.49.54,1.22,1.22,0,0,0,.15.17,1.06,1.06,0,0,0,1.45-.06l.11-.13a3.55,3.55,0,0,1,.47-.55,6,6,0,0,1,4.27-1.82h.06a6,6,0,0,1,5.75,7.87L18.26,15h0a1,1,0,0,0-.92.61L17,14.28a1,1,0,0,0-.61-.67,1,1,0,0,0-.91.11l-1.26.85-1.4-4a1,1,0,0,0-.8-.66,1,1,0,0,0-1,.4L7.57,15l-3.12.08A6.13,6.13,0,0,1,6,9.18Zm20,8.35a4.43,4.43,0,0,1-.55.49,1,1,0,0,0-.39.6l-8.93,9.12L6.84,18.65a1,1,0,0,0-.27-.33,4.23,4.23,0,0,1-.51-.44,5.65,5.65,0,0,1-.67-.81L8.11,17a1,1,0,0,0,.78-.41L11.52,13l1.2,3.45a1,1,0,0,0,.63.62,1,1,0,0,0,.88-.12l1.18-.8.79,2.9a1,1,0,0,0,.92.74h.05a1,1,0,0,0,.93-.64L18.93,17l7.41.09A4.58,4.58,0,0,1,26,17.53Z"></path>{" "}
        </g>{" "}
      </g>
    </svg>
  );
}

export function DrugAuthenticitySolutions() {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <path
          d="M11 9.5C11 8.34835 10.5607 7.1967 9.68198 6.31802C7.92462 4.56066 5.07538 4.56066 3.31802 6.31802C2.43934 7.1967 2 8.34835 2 9.5M11 9.5C11 10.6517 10.5607 11.8033 9.68198 12.682C7.92462 14.4393 5.07538 14.4393 3.31802 12.682C2.43934 11.8033 2 10.6517 2 9.5M11 9.5H2"
          stroke="#ec2088"
          strokeWidth="1.5"
          strokeLinecap="round"
        ></path>{" "}
        <path
          d="M14.7574 7.12132C15.9289 8.29289 17.8284 8.29289 19 7.12132C19.5858 6.53553 19.8787 5.76777 19.8787 5C19.8787 4.23223 19.5858 3.46447 19 2.87868C17.8284 1.70711 15.9289 1.70711 14.7574 2.87868C14.1716 3.46447 13.8787 4.23223 13.8787 5C13.8787 5.76777 14.1716 6.53553 14.7574 7.12132Z"
          stroke="#ec2088"
          strokeWidth="1.5"
          strokeLinecap="round"
        ></path>{" "}
        <path
          d="M20.9052 17.381C22.3649 15.9213 22.3649 13.5546 20.9052 12.0948C19.4454 10.6351 17.0787 10.6351 15.619 12.0948L12.0948 15.619C10.6351 17.0787 10.6351 19.4454 12.0948 20.9052C13.5546 22.3649 15.9213 22.3649 17.381 20.9052L20.9052 17.381Z"
          stroke="#ec2088"
          strokeWidth="1.5"
        ></path>{" "}
        <path
          d="M14 14C14 14 14.2813 15.4596 15.911 17.0892C17.5407 18.7189 19 19 19 19"
          stroke="#ec2088"
          strokeWidth="1.5"
        ></path>{" "}
      </g>
    </svg>
  );
}

export function DigitalIdVerification() {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <path
          d="M7 16V11.3615C7 10.8518 7.10026 10.3624 7.28451 9.90769M17 13.5V12.8154M9.22222 7.73446C10.0167 7.27055 10.9721 7 12 7C14.2795 7 16.2027 8.33062 16.8046 10.15"
          stroke="#ec2088"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></path>{" "}
        <path
          d="M10 17V14.8235M14 17V11.8529C14 10.8296 13.1046 10 12 10C10.8954 10 10 10.8296 10 11.8529V12.6471"
          stroke="#ec2088"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></path>{" "}
        <path
          d="M14 21.8C13.3538 21.9311 12.6849 22 12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 12.254 21.9905 12.5057 21.9719 12.7549"
          stroke="#ec2088"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></path>{" "}
        <path
          d="M21.1667 18.5H21.4C21.7314 18.5 22 18.7686 22 19.1V21.4C22 21.7314 21.7314 22 21.4 22H17.6C17.2686 22 17 21.7314 17 21.4V19.1C17 18.7686 17.2686 18.5 17.6 18.5H17.8333M21.1667 18.5V16.75C21.1667 16.1667 20.8333 15 19.5 15C18.1667 15 17.8333 16.1667 17.8333 16.75V18.5M21.1667 18.5H17.8333"
          stroke="#ec2088"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></path>{" "}
      </g>
    </svg>
  );
}

export function LastMileDeliveryInnovation() {
  return (
    <svg
      fill="#ec2088"
      className="size-10"
      version="1.1"
      id="Layer_1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g>
          {" "}
          <g>
            {" "}
            <path d="M484.981,331.558h-35.35v-94.442c0-13.092-10.651-23.742-23.742-23.742h-39.043c-4.372,0-7.914,3.543-7.914,7.914 c0,4.371,3.542,7.914,7.914,7.914h39.043c4.364,0,7.914,3.55,7.914,7.914v102.356c0,4.371,3.542,7.914,7.914,7.914h43.264 c4.364,0,7.914,3.55,7.914,7.914v60.675H440.95c-10.092-27.084-36.21-46.429-66.767-46.429c-30.572,0-56.7,19.363-66.781,46.467 c-0.248-0.023-0.498-0.038-0.752-0.038h-114.73c-10.092-27.084-36.21-46.429-66.767-46.429c-30.557,0-56.675,19.345-66.767,46.429 H19.104v-68.589H306.65c0.03,0,0.058-0.004,0.088-0.004c0.03,0,0.058,0.004,0.088,0.004c0.004,0,0.008,0,0.012,0 c4.372-0.006,7.909-3.555,7.903-7.926l-0.165-110.258h32.172c4.372,0,7.914-3.543,7.914-7.914c0-4.371-3.542-7.914-7.914-7.914 h-32.184v-110.27c0-4.371-3.542-7.914-7.914-7.914H196.991c22.483-10.69,48.726-24.034,55.95-31.259 c14.605-14.606,14.605-38.371,0-52.977c-14.605-14.606-38.371-14.605-52.976,0c-9.727,9.726-30.544,53.922-41.046,77.006 C148.417,64.876,127.6,20.68,117.873,10.954c-14.605-14.605-38.371-14.605-52.976,0c-7.075,7.075-10.972,16.482-10.972,26.488 c0,10.005,3.896,19.413,10.972,26.489c7.225,7.225,33.467,20.568,55.95,31.259H11.19c-4.371,0-7.914,3.543-7.914,7.914v88.638 c0,4.371,3.543,7.914,7.914,7.914c4.371,0,7.914-3.543,7.914-7.914v-80.724H125.68v220.54H19.104v-93.387 c0-4.371-3.543-7.914-7.914-7.914c-4.371,0-7.914,3.543-7.914,7.914v101.301v109.742c0,0.178,0.015,0.352,0.026,0.528 c-0.012,0.175-0.026,0.349-0.026,0.528c0,13.092,10.65,23.742,23.742,23.742h35.166C74.147,496.582,97.878,512,125.153,512 s51.006-15.418,62.969-37.988h123.094C323.179,496.582,346.909,512,374.185,512c27.282,0,51.018-15.425,62.977-38.005 c0.112,0.005,0.223,0.017,0.336,0.017h47.485c13.091,0,23.742-10.65,23.742-23.742v-26.38v-68.589 C508.724,342.209,498.073,331.558,484.981,331.558z M298.736,111.018v220.54H192.159v-220.54H298.736z M211.158,22.146 c4.086-4.086,9.518-6.336,15.296-6.336c5.778,0,11.211,2.251,15.296,6.336c8.434,8.434,8.434,22.158,0,30.593 c-6.104,6.103-37.961,21.914-65.054,34.461C189.245,60.108,205.054,28.25,211.158,22.146z M141.509,111.018h34.822v220.54h-34.822 V111.018z M76.091,52.739c-4.086-4.086-6.337-9.518-6.337-15.296c0-18.739,23.666-28.555,36.928-15.296 c6.104,6.104,21.914,37.961,34.462,65.054C114.052,74.651,82.195,58.842,76.091,52.739z M27.018,458.184 c-4.364,0-7.914-3.55-7.914-7.914c0-0.178-0.015-0.352-0.026-0.528c0.012-0.175,0.026-0.349,0.026-0.528v-17.411H54.51 c-0.371,2.941-0.585,5.929-0.585,8.969c0,6.005,0.756,11.836,2.161,17.411H27.018z M125.153,496.172 c-30.547,0-55.399-24.851-55.399-55.399s24.851-55.399,55.399-55.399s55.399,24.851,55.399,55.399S155.7,496.172,125.153,496.172z M194.219,458.184c1.406-5.575,2.161-11.406,2.161-17.411c0-3.04-0.213-6.028-0.585-8.969h107.746 c-0.371,2.941-0.585,5.929-0.585,8.969c0,6.005,0.755,11.836,2.161,17.411H194.219z M374.184,496.172 c-30.546,0-55.399-24.851-55.399-55.399s24.852-55.399,55.399-55.399s55.399,24.851,55.399,55.399 S404.731,496.172,374.184,496.172z M492.895,450.27c0,4.363-3.55,7.914-7.914,7.914H443.25c1.406-5.575,2.161-11.406,2.161-17.411 c0-3.04-0.213-6.028-0.585-8.969h48.069V450.27z"></path>{" "}
          </g>{" "}
        </g>{" "}
        <g>
          {" "}
          <g>
            {" "}
            <path d="M412.171,238.699h-75.975c-4.372,0-7.914,3.543-7.914,7.914v67.534c0,4.371,3.542,7.914,7.914,7.914h75.975 c4.372,0,7.914-3.543,7.914-7.914v-67.534C420.086,242.243,416.543,238.699,412.171,238.699z M404.257,306.233H344.11v-51.706 h60.147V306.233z"></path>{" "}
          </g>{" "}
        </g>{" "}
        <g>
          {" "}
          <g>
            {" "}
            <path d="M357.3,331.558h-21.104c-4.372,0-7.914,3.543-7.914,7.914c0,4.371,3.542,7.914,7.914,7.914H357.3 c4.372,0,7.914-3.543,7.914-7.914C365.214,335.102,361.672,331.558,357.3,331.558z"></path>{" "}
          </g>{" "}
        </g>{" "}
        <g>
          {" "}
          <g>
            {" "}
            <path d="M125.153,403.313c-20.656,0-37.46,16.804-37.46,37.46c0,20.656,16.804,37.46,37.46,37.46s37.46-16.804,37.46-37.46 C162.613,420.117,145.809,403.313,125.153,403.313z M125.153,462.405c-11.928,0-21.632-9.704-21.632-21.632 c0-11.928,9.704-21.632,21.632-21.632s21.632,9.704,21.632,21.632C146.785,452.701,137.081,462.405,125.153,462.405z"></path>{" "}
          </g>{" "}
        </g>{" "}
        <g>
          {" "}
          <g>
            {" "}
            <path d="M374.184,403.313c-20.656,0-37.46,16.804-37.46,37.46c0,20.656,16.804,37.46,37.46,37.46 c20.656,0,37.46-16.804,37.46-37.46C411.644,420.117,394.841,403.313,374.184,403.313z M374.184,462.405 c-11.928,0-21.632-9.704-21.632-21.632c0-11.928,9.704-21.632,21.632-21.632s21.632,9.704,21.632,21.632 C395.816,452.701,386.112,462.405,374.184,462.405z"></path>{" "}
          </g>{" "}
        </g>{" "}
      </g>
    </svg>
  );
}

export function PublicTransportOptimization() {
  return (
    <svg
      fill="#ec2088"
      viewBox="0 -24.7 122.88 122.88"
      version="1.1"
      id="Layer_1"
      xmlns="http://www.w3.org/2000/svg"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g>
          {" "}
          <path
            className="st0"
            style={{ fillRule: "evenodd", clipRule: "evenodd" }}
            d="M122.88,62.83h-3.37v6.53c0,2.27-1.86,4.13-4.13,4.13h-4.22c-2.27,0-4.13-1.86-4.13-4.13v-6.53H78.6v6.53 c0,2.27-1.86,4.13-4.13,4.13h-4.22c-2.27,0-4.13-1.86-4.13-4.13v-6.53h-3.26V13.76c0-4.77,2.78-8.24,8.38-10.39 c11.52-4.49,31.75-4.49,43.26,0c5.6,2.16,8.38,5.62,8.38,10.4V62.83L122.88,62.83L122.88,62.83z M53.18,45.88v18.86v4.48v4.27 h-6.45v-4.27H8.68v4.27H2.22v-4.27v-4.48V45.88H0v-4.24h5.43l3.03-9.42c0.89-2.77,2.38-5.29,5.29-5.29h6.66V23.2 c0-1.67-0.35-1.53,1.23-1.53h12.32c1.32,0,1.03-0.13,1.03,1.28v3.98h7.79c2.91,0,4.59,2.47,5.29,5.29l2.34,9.42h4.99v4.24 L53.18,45.88L53.18,45.88L53.18,45.88z M18.04,60.71H37.1v4.5H18.04V60.71L18.04,60.71L18.04,60.71z M4.38,50.24 c4.86,0.15,7.83,2.19,8.59,6.43H4.38V50.24L4.38,50.24L4.38,50.24z M50.41,50.24c-4.86,0.15-7.83,2.19-8.59,6.43h8.59V50.24 L50.41,50.24L50.41,50.24z M7.75,41.64h39.91l-1.71-7.9c-0.47-2.16-1.82-4.04-4.04-4.04h-27.6c-2.22,0-3.35,1.92-4.04,4.04 L7.75,41.64L7.75,41.64L7.75,41.64z M79.75,50.63h26.24c0.23,0,0.41,0.3,0.41,0.66v1.98c0,0.37-0.19,0.66-0.41,0.66H79.75 c-0.23,0-0.41-0.3-0.41-0.66v-1.98C79.33,50.93,79.52,50.63,79.75,50.63L79.75,50.63L79.75,50.63L79.75,50.63z M79.75,55.63h26.24 c0.23,0,0.41,0.3,0.41,0.66v1.98c0,0.36-0.19,0.66-0.41,0.66H79.75c-0.23,0-0.41-0.3-0.41-0.66v-1.98 C79.33,55.93,79.52,55.63,79.75,55.63L79.75,55.63L79.75,55.63z M68.27,14.04h21.4c0.66,0,1.21,0.54,1.21,1.21v17.68 c0,0.66-0.54,1.21-1.21,1.21h-21.4c-0.66,0-1.21-0.54-1.21-1.21V15.24C67.06,14.58,67.61,14.04,68.27,14.04L68.27,14.04 L68.27,14.04L68.27,14.04z M95.97,14.04h21.4c0.66,0,1.21,0.54,1.21,1.21v17.68c0,0.66-0.54,1.21-1.21,1.21h-21.4 c-0.66,0-1.21-0.54-1.21-1.21V15.24C94.77,14.58,95.31,14.04,95.97,14.04L95.97,14.04L95.97,14.04L95.97,14.04z M119.47,55.33 c0-1.23-0.42-2.26-1.27-3.1c-0.85-0.85-1.89-1.27-3.11-1.27c-1.19,0-2.21,0.43-3.08,1.27c-0.87,0.85-1.29,1.87-1.29,3.1 c0,1.19,0.43,2.2,1.29,3.04c0.87,0.85,1.89,1.27,3.08,1.27c1.21,0,2.26-0.42,3.11-1.27C119.05,57.53,119.47,56.53,119.47,55.33 L119.47,55.33L119.47,55.33L119.47,55.33z M75.01,55.33c0-1.23-0.43-2.26-1.27-3.1c-0.85-0.85-1.88-1.27-3.11-1.27 c-1.21,0-2.26,0.43-3.11,1.27c-0.85,0.85-1.25,1.87-1.25,3.1c0,1.19,0.4,2.2,1.25,3.04c0.85,0.85,1.89,1.27,3.11,1.27 c1.23,0,2.26-0.42,3.11-1.27C74.58,57.53,75.01,56.53,75.01,55.33L75.01,55.33L75.01,55.33L75.01,55.33z"
          ></path>{" "}
        </g>{" "}
      </g>
    </svg>
  );
}

export function AffordableHousingTechnology() {
  return (
    <svg
      fill="#ec2088"
      version="1.2"
      baseProfile="tiny"
      id="Layer_1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="-415 217 128 128"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <path
          id="Layer_1_1_"
          d="M-332.8,252.4v-6.5h-18.4h-0.1h-18.4v6.5h-18v91.7h27v-21.5h19.1v21.5h27v-91.7H-332.8z M-363.7,312.7 L-363.7,312.7h-12v-12.1h12C-363.7,300.6-363.7,312.7-363.7,312.7z M-363.7,294.5L-363.7,294.5h-12v-12h12 C-363.7,282.6-363.7,294.5-363.7,294.5z M-363.7,276.5h-12.1v-12h12v12H-363.7z M-344.8,312.7h-12.8v-12.1h12.8 C-344.8,300.6-344.8,312.7-344.8,312.7z M-344.8,294.5h-12.8v-12h6.4h0.1h6.4v12H-344.8z M-344.8,276.5h-6.4h-0.1h-6.4v-12h6.4h0.1 h6.4V276.5z M-326.8,312.7h-11l0,0h-1.1v-12.1h1.1h11V312.7z M-326.8,294.7h-11l0,0h-1.1v-12.1h12v12.1H-326.8z M-326.8,276.5h-12.1 v-12h12v12H-326.8z M-364.8,241.2c4.1,0,7.9-2.3,9.7-5.7h5.8l3.6-3.6l2.8,2.8l2.7-2.7l2.7,2.8l2.8-2.8l2.8,2.8l4.7-4.7 c0.7-0.7,0.7-1.1,0-1.9l-3.9-4h-24.1c-2-3.5-5.7-5.8-9.9-5.8c-6.2,0-11.3,5.1-11.3,11.4C-376.1,236.1-371,241.2-364.8,241.2z M-369.8,226.6c1.7,0,3,1.4,3,3.1s-1.4,3.1-3,3.1c-1.6,0-3-1.4-3-3.1C-372.7,228-371.5,226.6-369.8,226.6z"
        ></path>{" "}
      </g>
    </svg>
  );
}

export function DigitalArtsAndCultureMarket() {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="size-10"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <path
          d="M21.1935 16.793C20.8437 19.2739 20.6689 20.5143 19.7717 21.2572C18.8745 22 17.5512 22 14.9046 22H9.09536C6.44881 22 5.12553 22 4.22834 21.2572C3.33115 20.5143 3.15626 19.2739 2.80648 16.793L2.38351 13.793C1.93748 10.6294 1.71447 9.04765 2.66232 8.02383C3.61017 7 5.29758 7 8.67239 7H15.3276C18.7024 7 20.3898 7 21.3377 8.02383C22.0865 8.83268 22.1045 9.98979 21.8592 12"
          stroke="#ec2088"
          strokeWidth="1.5"
          strokeLinecap="round"
        ></path>{" "}
        <path
          d="M19.5617 7C19.7904 5.69523 18.7863 4.5 17.4617 4.5H6.53788C5.21323 4.5 4.20922 5.69523 4.43784 7"
          stroke="#ec2088"
          strokeWidth="1.5"
        ></path>{" "}
        <path
          d="M17.4999 4.5C17.5283 4.24092 17.5425 4.11135 17.5427 4.00435C17.545 2.98072 16.7739 2.12064 15.7561 2.01142C15.6497 2 15.5194 2 15.2588 2H8.74099C8.48035 2 8.35002 2 8.24362 2.01142C7.22584 2.12064 6.45481 2.98072 6.45704 4.00434C6.45727 4.11135 6.47146 4.2409 6.49983 4.5"
          stroke="#ec2088"
          strokeWidth="1.5"
        ></path>{" "}
        <circle
          cx="16.5"
          cy="11.5"
          r="1.5"
          stroke="#ec2088"
          strokeWidth="1.5"
        ></circle>{" "}
        <path
          d="M19.9999 20L17.1157 17.8514C16.1856 17.1586 14.8004 17.0896 13.7766 17.6851L13.5098 17.8403C12.7984 18.2542 11.8304 18.1848 11.2156 17.6758L7.37738 14.4989C6.6113 13.8648 5.38245 13.8309 4.5671 14.4214L3.24316 15.3803"
          stroke="#ec2088"
          strokeWidth="1.5"
          strokeLinecap="round"
        ></path>{" "}
      </g>
    </svg>
  );
}

export function Briefcase() {
  return (
    <svg
      fill="none"
      viewBox="0 0 60 60"
      id="Capa_1"
      version="1.1"
      xmlns="http://www.w3.org/2000/svg"
      height={50}
      width={50}
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="5" stroke="#ec2088"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g>
          {" "}
          <path
            d="M36,21H24v6h12V21z M34,25h-8v-2h8V25z"
            className="icon-shape"
          ></path>{" "}
          <circle cx="11" cy="26" r="1"></circle>{" "}
          <circle cx="11" cy="41" r="1" className="icon-shape"></circle>{" "}
          <circle cx="49" cy="26" r="1" className="icon-shape"></circle>{" "}
          <circle cx="49" cy="41" r="1" className="icon-shape"></circle>{" "}
          <path
            className="icon-shape"
            d="M57.2,13.4C55.997,12.497,54.505,12,53,12v-0.181C53,10.265,51.735,9,50.181,9h-2.361C46.265,9,45,10.265,45,11.819V12h-1 V9h-2V7.889C42,4.09,38.91,1,35.111,1H24.889C21.09,1,18,4.09,18,7.889V9h-2v3h-1v-0.181C15,10.265,13.735,9,12.181,9H9.819 C8.265,9,7,10.265,7,11.819V12c-1.505,0-2.997,0.497-4.2,1.4C1.047,14.715,0,16.809,0,19v31v1v2.578C0,56.567,2.432,59,5.422,59H8 h1h42h1h2.578C57.568,59,60,56.567,60,53.578V51v-1V19C60,16.809,58.953,14.715,57.2,13.4z M56,15c1.252,0.939,2,2.435,2,4v15h-3 v-3h-2V14C54.075,14,55.141,14.355,56,15z M47,39h4v3c0,1.103-0.897,2-2,2s-2-0.897-2-2V39z M52,37h-6h-1v-4h1h2v1 c0,0.553,0.448,1,1,1s1-0.447,1-1v-1h2h1v4H52z M47,11.819C47,11.367,47.368,11,47.819,11h2.361C50.632,11,51,11.367,51,11.819V13 v18h-1v-1c0-0.553-0.448-1-1-1s-1,0.447-1,1v1h-1V13V11.819z M42,12h-6v-1h6V12z M36,9h-2v3h-8V9h-2V7.444 c0-0.271,0.175-0.417,0.25-0.468c0.075-0.051,0.275-0.154,0.528-0.053c3.368,1.394,7.078,1.394,10.445,0 c0.251-0.102,0.452,0.002,0.528,0.053C35.825,7.027,36,7.173,36,7.444V9z M20,7.889C20,5.193,22.193,3,24.889,3h10.223 C37.807,3,40,5.193,40,7.889V9h-2V7.444c0-0.857-0.425-1.653-1.138-2.13c-0.712-0.478-1.611-0.566-2.403-0.238 c-2.876,1.188-6.043,1.188-8.918,0c-0.792-0.328-1.69-0.239-2.403,0.238C22.425,5.791,22,6.587,22,7.444V9h-2V7.889z M18,11h6v1h-6 V11z M16,14h10h8h10h1v17h-2v3H17v-3h-2V14H16z M9,39h4v3c0,1.103-0.897,2-2,2s-2-0.897-2-2V39z M14,37H8H7v-4h1h2v1 c0,0.553,0.448,1,1,1s1-0.447,1-1v-1h2h1v4H14z M9,11.819C9,11.367,9.368,11,9.819,11h2.361C12.632,11,13,11.367,13,11.819V13v18 h-1v-1c0-0.553-0.448-1-1-1s-1,0.447-1,1v1H9V13V11.819z M4,15c0.859-0.645,1.925-1,3-1v17H5v3H2V19C2,17.435,2.748,15.939,4,15z M2,53.578V52h1.388C5.379,52,7,53.62,7,55.612V57H5.422C3.535,57,2,55.465,2,53.578z M3.388,50H2V36h3v3h2v3c0,2.206,1.794,4,4,4 s4-1.794,4-4v-3h2v-3h12v21H9v-1.388C9,52.518,6.482,50,3.388,50z M58,53.578C58,55.465,56.465,57,54.578,57H53v-1.388 C53,53.62,54.621,52,56.612,52H58V53.578z M56.612,50C53.518,50,51,52.518,51,55.612V57H31V36h12v3h2v3c0,2.206,1.794,4,4,4 s4-1.794,4-4v-3h2v-3h3v14H56.612z"
            stroke="#ec2088"
            strokeWidth="1"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>{" "}
        </g>{" "}
        <g></g> <g></g> <g></g> <g></g> <g></g> <g></g> <g></g> <g></g> <g></g>{" "}
        <g></g> <g></g> <g></g> <g></g> <g></g> <g></g>{" "}
      </g>
    </svg>
  );
}

export function Users() {
  return (
    <svg
      version="1.1"
      id="Icons"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 32 32"
      fill="none"
      stroke="#ec2088"
      strokeWidth="1"
      strokeLinecap="round"
      strokeLinejoin="round"
      height={50}
      width={50}
      className="benefits-icon"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g>
          {" "}
          <path
            className="icon-shape"
            d="M18.8,21.5c0.3-0.5,0.1-1.1-0.3-1.4L17,19.2V17c0-0.6-0.4-1-1-1s-1,0.4-1,1v2.2l-1.5,0.9c-0.5,0.3-0.6,0.9-0.3,1.4 c0.3,0.5,0.9,0.6,1.4,0.3l1.4-0.9c0,0,0.1,0,0.1,0s0.1,0,0.1,0l1.4,0.9C17.6,22,17.8,22,18,22C18.3,22,18.7,21.8,18.8,21.5z"
          ></path>{" "}
          <path
            className="icon-shape"
            d="M11,14l10,0c0.3,0,0.6-0.1,0.8-0.4c0.2-0.2,0.3-0.5,0.2-0.8c-0.3-1.5-1.4-2.7-2.9-3.1c-0.4-0.1-0.7-0.4-0.8-0.8 C18.7,8.4,19,7.7,19,7V5c0-1.7-1.3-3-3-3s-3,1.3-3,3v2c0,0.7,0.3,1.4,0.7,1.9c-0.1,0.4-0.4,0.7-0.8,0.8c-1.5,0.4-2.6,1.6-2.8,3.1 c-0.1,0.3,0,0.6,0.2,0.8C10.4,13.9,10.7,14,11,14z"
          ></path>{" "}
          <path
            className="icon-shape"
            d="M27.1,25.7c-0.4-0.1-0.7-0.4-0.8-0.8c0.4-0.5,0.7-1.2,0.7-1.9v-2c0-1.7-1.3-3-3-3s-3,1.3-3,3v2c0,0.7,0.3,1.4,0.7,1.9 c-0.1,0.4-0.4,0.7-0.8,0.8c-1.5,0.4-2.6,1.6-2.8,3.1c-0.1,0.3,0,0.6,0.2,0.8c0.2,0.2,0.5,0.4,0.8,0.4l10,0c0.3,0,0.6-0.1,0.8-0.4 c0.2-0.2,0.3-0.5,0.2-0.8C29.7,27.3,28.5,26.1,27.1,25.7z"
          ></path>{" "}
          <path
            className="icon-shape"
            d="M11.1,25.7c-0.4-0.1-0.7-0.4-0.8-0.8c0.4-0.5,0.7-1.2,0.7-1.9v-2c0-1.7-1.3-3-3-3s-3,1.3-3,3v2c0,0.7,0.3,1.4,0.7,1.9 c-0.1,0.4-0.4,0.7-0.8,0.8c-1.5,0.4-2.6,1.6-2.8,3.1c-0.1,0.3,0,0.6,0.2,0.8C2.4,29.9,2.7,30,3,30l10,0c0.3,0,0.6-0.1,0.8-0.4 c0.2-0.2,0.3-0.5,0.2-0.8C13.7,27.3,12.5,26.1,11.1,25.7z"
          ></path>{" "}
        </g>{" "}
      </g>
    </svg>
  );
}

export function ChartUp() {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      height={50}
      width={50}
      className="benefits-icon"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <path
          className="icon-shape"
          d="M21 21H7.8C6.11984 21 5.27976 21 4.63803 20.673C4.07354 20.3854 3.6146 19.9265 3.32698 19.362C3 18.7202 3 17.8802 3 16.2V3M6 15L10 11L14 15L20 9M20 9V13M20 9H16"
          stroke="#ec2088"
          strokeWidth="0.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></path>{" "}
      </g>
    </svg>
  );
}

export function Confidence() {
  return (
    <svg
      fill="none"
      version="1.1"
      id="Capa_1"
      xmlns="http://www.w3.org/2000/svg"
      strokeLinecap="round"
      strokeWidth="0.5"
      strokeLinejoin="round"
      stroke="#ec2088"
      height={50}
      width={50}
      className="benefits-icon"
    >
      <g id="SVGRepo_bgCarrier"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g>
          {" "}
          <g>
            {" "}
            <path
              className="icon-shape"
              d="M1.434,28.084c1.527,0.188,3.644,0.847,3.644,1.587c0,0.207,0.128,0.393,0.321,0.467c0.189,0.073,0.412,0.021,0.551-0.133 c0.026-0.027,2.688-2.917,7.878-2.917c5.405,0,8.5,2.556,8.5,3.25c0,0.241,0.172,0.447,0.408,0.491 c0.24,0.044,0.471-0.087,0.559-0.312c0.019-0.051,1.904-4.85,4.922-4.85c0.01,0,0.021,0,0.027,0 c1.557,0.018,4.362,0.042,6.073,2.143c0.193,0.269,0.318,0.438,0.354,0.486c0.16,0.223,0.467,0.271,0.688,0.116 s0.281-0.456,0.136-0.686c-0.118-0.184-0.243-0.354-0.372-0.518c-1.658-2.314-8.548-12.151-8.548-15.794 c0-0.194-0.112-0.371-0.288-0.453c-0.175-0.083-0.382-0.056-0.531,0.068c-0.058,0.047-5.736,4.688-10.149,1.041 c-0.052-0.056-1.274-1.379-0.466-2.437c0.094-0.123,0.124-0.282,0.085-0.432s-0.146-0.272-0.289-0.333 c-0.008-0.003-0.812-0.354-1.062-1.093c-0.164-0.483-0.062-1.055,0.304-1.7c1.356-2.392,3.701-4.431,3.779-4.506 c0.012-0.014,1.186-1.341,3.882,0.116c0.351,0.189,0.768,0.409,1.236,0.656c3.612,1.901,10.334,5.439,12.925,9.109 c0.548,0.773,1.248,1.658,2.062,2.682c3.635,4.578,9.113,11.486,9.43,19.683c-0.25,0.541-1.715,3.412-5.574,5.773 c-3.948,2.418-11.291,4.786-23.824,2.332c-0.13-0.025-0.272,0.002-0.385,0.082c-0.113,0.079-0.187,0.2-0.206,0.338 c-0.006,0.039-0.591,3.967-2.71,6.318c-0.185,0.205-0.168,0.521,0.037,0.706c0.096,0.086,0.216,0.128,0.335,0.128 c0.137,0,0.272-0.056,0.372-0.166c1.867-2.074,2.626-5.123,2.865-6.331c3.566,0.67,6.729,0.956,9.533,0.956 c16.356-0.001,20.479-9.74,20.523-9.854c0.026-0.064,0.038-0.135,0.036-0.203c-0.276-8.578-5.917-15.683-9.646-20.385 c-0.806-1.012-1.496-1.887-2.025-2.638c-2.729-3.866-9.592-7.477-13.278-9.417c-0.465-0.245-0.879-0.463-1.228-0.651 c-3.397-1.837-5.051,0.036-5.063,0.062c-0.102,0.087-2.499,2.169-3.941,4.715c-0.515,0.908-0.643,1.758-0.378,2.525 c0.237,0.689,0.741,1.144,1.126,1.405c-0.558,1.27,0.255,2.644,0.868,3.285c4.082,3.376,8.941,0.736,10.749-0.476 c0.651,3.412,4.48,9.436,6.937,13.036c-1.628-0.668-3.354-0.686-4.443-0.695c-2.725-0.035-4.565,2.925-5.363,4.513 c-1.268-1.495-4.704-3.096-8.974-3.096c-4.277,0-6.947,1.781-8.048,2.708c-0.904-1.229-3.599-1.627-4.225-1.704 c-0.268-0.035-0.522,0.161-0.557,0.437C0.965,27.799,1.16,28.05,1.434,28.084z"
            ></path>{" "}
            <path
              className="icon-shape"
              d="M30.738,39.531c1.036,0,2.114-0.15,3.146-0.551c1.934-0.75,3.356-2.25,4.239-4.457c0.104-0.256-0.021-0.547-0.276-0.65 c-0.256-0.103-0.547,0.021-0.647,0.279c-0.771,1.938-2.013,3.248-3.671,3.895c-3.604,1.402-8.104-0.721-8.146-0.743 c-0.25-0.119-0.549-0.015-0.668,0.233c-0.12,0.249-0.016,0.547,0.233,0.667C25.093,38.275,27.739,39.531,30.738,39.531z"
            ></path>{" "}
          </g>{" "}
        </g>{" "}
      </g>
    </svg>
  );
}

export function HandLove() {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      strokeLinecap="round"
      strokeWidth="0.5"
      strokeLinejoin="round"
      stroke="#ec2088"
      height={50}
      width={50}
      className="benefits-icon"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <path
          className="icon-shape"
          d="M6 20.087H8.61029C8.95063 20.087 9.28888 20.1275 9.61881 20.2085L12.3769 20.8788C12.9753 21.0246 13.5988 21.0387 14.2035 20.9213L17.253 20.328C18.0585 20.1711 18.7996 19.7853 19.3803 19.2204L21.5379 17.1216C22.154 16.5233 22.154 15.5523 21.5379 14.953C20.9832 14.4133 20.1047 14.3526 19.4771 14.8102L16.9626 16.6447C16.6025 16.908 16.1643 17.0497 15.7137 17.0497H13.2855L14.8311 17.0497C15.7022 17.0497 16.4079 16.3632 16.4079 15.5158V15.209C16.4079 14.5054 15.9156 13.8919 15.2141 13.7218L12.8286 13.1416C12.4404 13.0475 12.0428 12.9999 11.6431 12.9999C10.6783 12.9999 8.93189 13.7987 8.93189 13.7987L6 15.0248M2 14.5999L2 20.3999C2 20.9599 2 21.24 2.10899 21.4539C2.20487 21.642 2.35785 21.795 2.54601 21.8909C2.75992 21.9999 3.03995 21.9999 3.6 21.9999H4.4C4.96005 21.9999 5.24008 21.9999 5.45399 21.8909C5.64215 21.795 5.79513 21.642 5.89101 21.4539C6 21.24 6 20.9599 6 20.3999V14.5999C6 14.0398 6 13.7598 5.89101 13.5459C5.79513 13.3577 5.64215 13.2048 5.45399 13.1089C5.24008 12.9999 4.96005 12.9999 4.4 12.9999H3.6C3.03995 12.9999 2.75992 12.9999 2.54601 13.1089C2.35785 13.2048 2.20487 13.3577 2.10899 13.5459C2 13.7598 2 14.0398 2 14.5999ZM17.1914 3.59215C16.5946 2.34329 15.2186 1.68168 13.8804 2.32027C12.5423 2.95886 11.9722 4.47328 12.5325 5.80272C12.8787 6.62435 13.8707 8.2199 14.5781 9.31893C14.8394 9.725 14.9701 9.92804 15.161 10.0468C15.3247 10.1487 15.5297 10.2036 15.7224 10.1972C15.9471 10.1898 16.1618 10.0793 16.5911 9.85832C17.7532 9.26021 19.4101 8.37445 20.1208 7.83602C21.2707 6.96481 21.5556 5.36347 20.6947 4.14614C19.8337 2.9288 18.3327 2.80902 17.1914 3.59215Z"
        ></path>{" "}
      </g>
    </svg>
  );
}

export function Certificate() {
  return (
    <svg
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      strokeLinecap="round"
      strokeWidth="0.5"
      strokeLinejoin="round"
      stroke="#ec2088"
      height={50}
      width={50}
      className="benefits-icon"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <path
          className="icon-shape"
          d="M23 1v18h-3v-1h2V2H2v16h8v1H1V1zm-7 2H8v1h8zm-2 3V5h-4v1zm-7 5H3v1h4zm0 2H3v1h4zm-4 3h2v-1H3zm14-3a2 2 0 1 1-2-2 2.002 2.002 0 0 1 2 2zm-1 0a1 1 0 1 0-1 1 1.001 1.001 0 0 0 1-1zm.002-4.293a.965.965 0 0 0 1.32.55 1.08 1.08 0 0 1 1.213.207 1.066 1.066 0 0 1 .21 1.21.966.966 0 0 0 .548 1.324 1.064 1.064 0 0 1 0 2.004.965.965 0 0 0-.549 1.323A1.05 1.05 0 0 1 18 16.816v7.046l-3-2.538-3 2.538v-7.046a1.05 1.05 0 0 1-.744-1.49.965.965 0 0 0-.549-1.324 1.064 1.064 0 0 1 0-2.004.966.966 0 0 0 .549-1.324 1.066 1.066 0 0 1 .209-1.21 1.08 1.08 0 0 1 1.212-.206.965.965 0 0 0 1.32-.551 1.064 1.064 0 0 1 2.005 0zm.998 13v-5.04a.93.93 0 0 0-.998.625 1.064 1.064 0 0 1-2.004 0 .93.93 0 0 0-.998-.625v5.039l2-1.692zm-1.94-4.749a1.967 1.967 0 0 1 1.853-1.308 2.12 2.12 0 0 1 .87.197l.058-.091a1.964 1.964 0 0 1 1.116-2.695v-.122a1.966 1.966 0 0 1-1.116-2.695l-.087-.084a1.965 1.965 0 0 1-2.694-1.117h-.12a1.965 1.965 0 0 1-2.694 1.117l-.087.084a1.966 1.966 0 0 1-1.116 2.695v.122a1.964 1.964 0 0 1 1.116 2.695l.058.09a2.12 2.12 0 0 1 .87-.196 1.967 1.967 0 0 1 1.853 1.308L15 17z"
        ></path>
      </g>
    </svg>
  );
}

export const svgCollection: ReactElement[] = [
  <PrecisionFarmingSolution />,
  <ColdChainProduce />,
  <VocationalEdtech />,
  <LanguageLearning />,
  <OffGridPowerSolution />,
  <WasteToWasteEcosystem />,
  <WaterQualityAndMonitoring />,
  <MicroLendingForBusinesses />,
  <TelemedicineForRuralAreas />,
  <DrugAuthenticitySolutions />,
  <DigitalIdVerification />,
  <LastMileDeliveryInnovation />,
  <PublicTransportOptimization />,
  <AffordableHousingTechnology />,
  <DigitalArtsAndCultureMarket />,
];
